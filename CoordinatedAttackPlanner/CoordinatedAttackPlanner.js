(function() {
    ...
    function bindPlanDesignEvents() {
        ...
        // Add tribe button
        document.getElementById('cap-add-tribe').onclick = showAddTribeDialog;

        // Add all villages from selected players
    ...
    function addTargetPlayer() {
        ...
        if (targetPlayers.has(playerName)) {
        ...
        validatePlayer(playerName)
            .then(() => {
                ...
                // Store in recent targets (for next time)
                addToRecentTargets(playerName);
    ...
    function updateTargetPlayersDisplay() {
        ...
        targetPlayers.forEach(playerName => {
            html += `
                <span class="cap-target-item">
                    ${playerName}
                    <span class="remove" onclick="removeTargetPlayer('${playerName}')" style="color: red; cursor: pointer; margin-left: 5px; font-weight: bold;">&times;</span>
                </span>
            `;
        });
    ...
    window.removeTargetPlayer = function(playerName) {
        targetPlayers.delete(playerName);
        updateTargetPlayersDisplay();
    };

    // Show add tribe dialog
    function showAddTribeDialog() {
        const html = `
            <div style="padding: 20px;">
                ...
                <br>
                <button class="cap-button" onclick="addTribeMembers()">Add Tribe</button>
    ...
    window.addTribeMembers = function() {
    ...
    function validateTribe(tribeTag) {
        return new Promise((resolve, reject) => {
    ...
    function getTribeMembers(tribeId, tribeTag) {
        return new Promise((resolve, reject) => {
    ...
    function getTribeMembersBySearch(tribeTag) {
        return new Promise((resolve, reject) => {
